...
static stZSDLastPreviewSetting g_stMT6577ZSDLastPreview =
                    {
                        .SpinLock   = __SPIN_LOCK_UNLOCKED(g_stMT6577ZSDLastPreview.SpinLock),
                        .u4Dirty    =   0,
                    };
...
static inline int DescQueueRefill(void)
{
    ...
    #define _CHECK_Q_BUSY_AND_RETURN_( _seg_name_ ) \
        if( _DescWriteWaitBusy( RGB_ROT0_BASE ) < 0 ){\
            ...
            return -1; }
    stZSDLastPreviewReg * pstReg = &g_stMT6577ZSDLastPreview.stReg;
...
static int MT6577_SetZSDLastPreview(stZSDLastPreviewReg * a_u4Param)
{
    unsigned long   flag;

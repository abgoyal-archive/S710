...
{    
    ...
    if (g_enable)
        cpu_opp_limit(EVENT_SCREEN_ON_OFF, DVFS_F1, 1, true);
...
{    
    ...
    if (g_enable)
        cpu_opp_limit(EVENT_SCREEN_ON_OFF, DVFS_F1, 1, false);
...
void mtk_hotplug_mechanism_thermal_protect(int limited_cpus)
{
    ...
    HOTPLUG_INFO("mtk_hotplug_mechanism_thermal_protect: %d", limited_cpus);

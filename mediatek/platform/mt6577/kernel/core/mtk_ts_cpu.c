...
static char g_bind1[20]="mtktscpu-250x1";
static char g_bind2[20]="mtktscpu-500x1";
static char g_bind3[20]="mtktscpu-667x1";
static char g_bind4[20]="mtktscpu-750x1";
static char g_bind5[20]="mtktscpu-834x1";
static char g_bind6[20]="mtktscpu-1000x1";
static char g_bind7[20]="mtktscpu-667x2";
static char g_bind8[20]="mtktscpu-750x2";
static char g_bind9[20]="mtktscpu-834x2";
...
void disable_old_cpu_management(void)
{
		...
		if(previous_step==0)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F1 ,2, false);
		else if(previous_step==1)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F2 ,2, false);
		else if(previous_step==2)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F3 ,2, false);
		else if(previous_step==3)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F4 ,2, false);
		else if(previous_step==4)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F1 ,1, false);
		else if(previous_step==5)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F2 ,1, false);
		else if(previous_step==6)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F3 ,1, false);
		else if(previous_step==7)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F4 ,1, false);
		else if(previous_step==8)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F5 ,1, false);
		else if(previous_step==9)
				cpu_opp_limit(EVENT_THERMAL,DVFS_F6 ,1, false);		
}

//static int	now_freq=DVFS_F1, now_cpus=2;
static int _mtktscpu_set_power_consumption_state(void)
{
    ...
    else
    {
		...
		if (1 == cl_dev_F6x1_state)
		{
				...
				if(9 == previous_step)
						return 0;
				cpu_opp_limit(EVENT_THERMAL,DVFS_F6 ,1, true);
		...
		else if (1 == cl_dev_F5x1_state)
		{
				...
				if(8 == previous_step)
						return 0;
				cpu_opp_limit(EVENT_THERMAL,DVFS_F5 ,1, true);
				disable_old_cpu_management();		
				previous_step=8;
				xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
		}		
		else if (1 == cl_dev_F4x1_state)
		{
				...
				if(7 == previous_step)
						return 0;
				cpu_opp_limit(EVENT_THERMAL,DVFS_F4 ,1, true);
				disable_old_cpu_management();				
				previous_step=7;
				xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
		}
		...
		{	
				...
				if(6 == previous_step)
						return 0;
				cpu_opp_limit(EVENT_THERMAL,DVFS_F3 ,1, true);
				disable_old_cpu_management();
				previous_step=6;
				xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
		}		
		else if (1 == cl_dev_F2x1_state)
		{
				...
				if(5 == previous_step)
						return 0;
				cpu_opp_limit(EVENT_THERMAL,DVFS_F2 ,1, true);
				disable_old_cpu_management();
				previous_step=5;
				xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
		}
		...
		{		
				...
				if(4 == previous_step)
						return 0;
				cpu_opp_limit(EVENT_THERMAL,DVFS_F1 ,1, true);
				disable_old_cpu_management();
				previous_step=4;
				xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
		}
		else if (1 == cl_dev_F4x2_state)
    {
    		//now_step=3;
				if(3 == previous_step)
						return 0;
    		cpu_opp_limit(EVENT_THERMAL,DVFS_F4 ,2, true);
    		disable_old_cpu_management();
    		previous_step=3;
    		xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
    }
    else if (1 == cl_dev_F3x2_state)
    {
    		//now_step=2;
				if(2 == previous_step)
						return 0;
    		cpu_opp_limit(EVENT_THERMAL,DVFS_F3 ,2, true);
				disable_old_cpu_management();
    		previous_step=2;
    		xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
    }
    else if (1 == cl_dev_F2x2_state)
    {
    		//now_step=1;
				if(1 == previous_step)
						return 0;
    		cpu_opp_limit(EVENT_THERMAL,DVFS_F2 ,2, true);
    		disable_old_cpu_management();		
    		previous_step=1;
    		xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
    }
    else
    {
				...
				if(0 == previous_step)
						return 0;
				cpu_opp_limit(EVENT_THERMAL,DVFS_F1 ,2, true);
				disable_old_cpu_management();
				previous_step=0;
				xlog_printk(ANDROID_LOG_INFO, "Power/CPU_Thermal", "opp=%d\n", previous_step);
    }
...
{
		cl_dev_F1x2 = mtk_thermal_cooling_device_register("mtktscpu-1000x2", NULL,
						 &mtktscpu_cooling_F1x2_ops);
		cl_dev_F2x2 = mtk_thermal_cooling_device_register("mtktscpu-834x2", NULL,
						 &mtktscpu_cooling_F2x2_ops);
		cl_dev_F3x2 = mtk_thermal_cooling_device_register("mtktscpu-750x2", NULL,
						 &mtktscpu_cooling_F3x2_ops);				 
		cl_dev_F4x2 = mtk_thermal_cooling_device_register("mtktscpu-667x2", NULL,
						 &mtktscpu_cooling_F4x2_ops);
		cl_dev_F1x1 = mtk_thermal_cooling_device_register("mtktscpu-1000x1", NULL,
						 &mtktscpu_cooling_F1x1_ops);
		cl_dev_F2x1 = mtk_thermal_cooling_device_register("mtktscpu-834x1", NULL,
						 &mtktscpu_cooling_F2x1_ops);
		cl_dev_F3x1 = mtk_thermal_cooling_device_register("mtktscpu-750x1", NULL,
						 &mtktscpu_cooling_F3x1_ops);
		cl_dev_F4x1 = mtk_thermal_cooling_device_register("mtktscpu-667x1", NULL,
						 &mtktscpu_cooling_F4x1_ops);
		cl_dev_F5x1 = mtk_thermal_cooling_device_register("mtktscpu-500x1", NULL,
						 &mtktscpu_cooling_F5x1_ops);
		cl_dev_F6x1 = mtk_thermal_cooling_device_register("mtktscpu-250x1", NULL,
